...
def cale_mix(str_expire):
    ...
    while len(re.findall("[-+*/]", str_expire[1:])) != 0:
        if len(re.findall("(\*|/)", str_expire)) != 0:
            str_expire = str_expire.replace(mul_div.search(str_expire).group(),
                                            touble_cale(mul_div.search(str_expire).group()))
        elif len(re.findall("(\+|-)", str_expire)) != 0:
            str_expire = str_expire.replace(plus_minus.search(str_expire).group(),
                                            touble_cale(plus_minus.search(str_expire).group()))
    return str_expire
...
def remove_bracket(str_expire):
    ...
    if len(bracket.findall(str_expire)) == 0:
        ...
        while len(bracket.findall(str_expire)) != 0:
            ...
            str_expire = str_expire.replace(bracket.search(str_expire).group(),
                                            cale_mix(judge_mul_minus(bracket.search(str_expire).group()[1:-1])))
